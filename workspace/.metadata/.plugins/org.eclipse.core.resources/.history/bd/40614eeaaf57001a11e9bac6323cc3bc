...
public class BoardListCommand implements BoardCommand {
	...
	public void execute(SqlSession sqlSession, Model model, HttpServletRequest request) {
		...
		// 전체 게시글 개수 구하기
		int totalRecord = bDao.select_total_record();
		
		// ◀ 1 2 3 ▶ 생성 (pagingView)
		String pagingView = Paging.getPaging("boardList", nowPage, recordPerPage, totalRecord, 0, "0", "");
		
		// request 에 pagingView 저장
		session.setAttribute("pagingView", pagingView);
		
		ArrayList<BoardDto> list = bDao.list(map);
