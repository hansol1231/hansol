...
public class AdminQueryCommand implements AdminCommand {
	...
	public void execute(SqlSession sqlSession, Model model, HttpServletRequest request) {
		...
		int totalRecord = aDao.select_search_record(sel, "%" + search + "%");
		
		// ◀ 1 2 3 ▶ 생성 (pagingView)
		String pagingView = Paging.getPaging("dynamicQueryList", nowPage, recordPerPage, totalRecord, 0, sel, search);
		
		// request 에 pagingView 저장
		session.setAttribute("pagingView", pagingView);
		
		ArrayList<MemberDto> list = aDao.dynamicQueryList(sel, "%" + search + "%", begin, end);
